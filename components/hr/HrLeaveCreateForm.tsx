  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (typeRequiresAttachment(leaveType)) {
      ...
      if (!(file instanceof File) || file.size <= 0) {
        toast.error("Upload a supporting document (PNG, JPEG, or PDF).");
        setBusy(false);
        return;
      }
      if (file.size > MC_DOCUMENT_MAX_BYTES) {
        toast.error(
          `File too large (${formatBytes(file.size)}). Max ${MC_DOCUMENT_MAX_SIZE_LABEL}.`,
        );
        setBusy(false);
        return;
      }
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(json?.message ?? json?.error ?? "Could not record leave.");
        return;
      }
      form.reset();
      setLeaveType("annual");
  ...
  return (
    <form id={formId} onSubmit={onSubmit} className="space-y-5">
      ...
      <section className="space-y-3 border-t border-cream-200 pt-4 dark:border-hairline-dark">
        ...
        <FormField label="Team member" htmlFor="leave-employee" required>
          <Select
            id="leave-employee"
            name="employee_id"
            required
      ...
      <section className="space-y-3 border-t border-cream-200 pt-4 dark:border-hairline-dark">
        ...
        <FormField label="Notes" htmlFor="leave-reason" hint="Optional">
          <Textarea
            id="leave-reason"
            name="reason"
            maxLength={500}
      ...
      {typeRequiresAttachment(leaveType) ? (
        <section className="space-y-2 rounded-lg border border-amber-200/80 bg-amber-50/50 p-3 dark:border-amber-900/40 dark:bg-amber-950/20">
          ...
          <FormField htmlFor="leave-mc-doc">
            <input
              id="leave-mc-doc"
              name="mc_document"
              type="file"
