...
import { OPERATIONS_ASSISTANT_SUGGESTIONS } from "@/lib/ai/operations-assistant-prompt";
import type {
...
import { AimanAssistantMessage } from "@/components/operations/AimanAssistantMessage";
...
function storageKey(businessId: string): string {
  return `bn-aiman-assistant-chat-v1-${businessId}`;
}

export type AimanAssistantStatus = PillarAssistantStatus;
export type AimanAssistantChatHandle = PillarAssistantChatHandle;

interface AimanAssistantChatProps {
  businessId: string;
...
export const AimanAssistantChat = forwardRef<
  AimanAssistantChatHandle,
  AimanAssistantChatProps
>(function AimanAssistantChat(
  {
    businessId,
    initialStatus,
    initialSeed,
  ...
  const newChat = useCallback(() => {
    ...
    void fetch("/api/operations/assistant", { method: "DELETE" }).catch(
      () => undefined,
    );
  }, [businessId]);
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || "Could not reach Aiman. Try again.");
      }
  ...
  if (!status.assistant_enabled) {
    return (
      ...
      >
        Aiman is turned off. Enable it in{" "}
  ...
  const displayName = status.display_name || "Aiman";
      ...
      >
        {turns.length === 0 ? (
          ...
          >
            <p className="text-sm font-semibold text-ink dark:text-cream-100">
              Ask {displayName} about your Operations records
            </p>
            <p className="mt-1 max-w-sm text-xs text-ink-muted dark:text-cream-400">
              Products, orders, and bookings — clarifying questions are free;
              plans and advice use credits.
            </p>
            <div className={cn("flex flex-wrap justify-center gap-2", isPanel ? "mt-4" : "mt-5")}>
              {OPERATIONS_ASSISTANT_SUGGESTIONS.map((prompt) => (
                <button
                  key={prompt}
                  type="button"
                  disabled={loading}
          ...
          turns.map((turn, i) => (
            ...
            >
              {turn.role === "user" ? (
                ...
                <AimanAssistantMessage content={turn.content} />
              )}
            </div>
          ))
        )}
        {loading ? (
          <div className="flex items-center gap-2 text-xs text-ink-muted dark:text-cream-400">
            ...
            Checking your Operations records…
          </div>
        ) : null}
      </div>
      ...
      >
        <div className="flex items-end gap-2">
          <textarea
            ...
            disabled={loading}
            className={cn(
              "flex-1 resize-none border border-[#E5E0D8] bg-white text-sm text-ink placeholder:text-ink-subtle focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 disabled:opacity-60 dark:border-hairline-dark dark:bg-panel-dark dark:text-cream-100",
              isPanel
                ? "max-h-28 min-h-[40px] rounded-lg px-3 py-2"
          ...
          <button
            ...
            className={cn(
              "inline-flex shrink-0 items-center justify-center bg-brand-500 text-white transition-colors hover:bg-brand-600 disabled:opacity-50",
              isPanel
        ...
        {!isPanel ? (
          <p className="mt-2 text-[11px] text-ink-muted dark:text-cream-500">
            ...
            free; advice and actions use credits.
          </p>
        ) : null}
      </form>
    </div>
