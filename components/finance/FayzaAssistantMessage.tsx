...
function renderInline(text: string, keyPrefix: string): ReactNode[] {
  ...
  while ((match = pattern.exec(text)) !== null) {
    ...
    if (token.startsWith("**")) {
      ...
      if (parsed && isSafeInternalHref(parsed.href)) {
        nodes.push(
          <Link
...
function renderList(lines: string[], keyPrefix: string, card = false): ReactNode {
  const list = (
...
function renderBlock(block: string, index: number): ReactNode {
      ...
      <div key={`success-${index}`} className="space-y-3">
        <div className="flex items-start gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3.5 py-2.5 dark:border-emerald-900/50 dark:bg-emerald-950/40">
          ...
          <p className="text-sm font-semibold text-emerald-900 dark:text-emerald-100">
            {renderInline(firstLine.replace(/^✅\s*/, ""), `success-${index}`)}
          </p>
        </div>
  ...
  const summaryCard =
    introLines.some((line) => /ringkasan|summary|transaksi/i.test(line)) &&
    listLines.length > 0;
      ...
      <div key={`block-${index}`} className="my-3 first:mt-0 last:mb-0">
        {introLines.map((line, li) =>
          isSectionHeading(line) ? (
            ...
            >
              {renderInline(line.replace(/\*\*/g, ""), `h-${index}-${li}`)}
            </p>
          ) : (
        ...
        {renderList(listLines, `block-${index}`, summaryCard)}
      </div>
    );
  }
...
interface FayzaAssistantMessageProps {
  content: string;
  className?: string;
}

/** Rich Markdown for Fayza — success banners, section cards, internal links. */
export function FayzaAssistantMessage({
  content,
  className,
