...
function storageKey(businessId: string): string {
  return `bn-fayza-assistant-chat-v1-${businessId}`;
}

export type FayzaAssistantStatus = PillarAssistantStatus;
export type FayzaAssistantChatHandle = PillarAssistantChatHandle;

interface FayzaAssistantChatProps {
  businessId: string;
...
export const FayzaAssistantChat = forwardRef<
  FayzaAssistantChatHandle,
  FayzaAssistantChatProps
>(function FayzaAssistantChat(
  {
    businessId,
    initialStatus,
    initialSeed,
  ...
  const newChat = useCallback(() => {
    ...
    void fetch("/api/finance/assistant", { method: "DELETE" }).catch(
      () => undefined,
    );
  }, [businessId]);
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || "Could not reach Fayza. Try again.");
      }
  ...
  if (!status.assistant_enabled) {
    return (
      ...
      >
        Fayza is turned off. Enable it in{" "}
  ...
  const displayName = status.display_name || "Fayza";
          ...
          <textarea
            ...
            className={cn(
              ...
              isPanel
                ...
                : "max-h-32 min-h-[42px] rounded-xl px-3.5 py-2.5",
            )}
          />
          <button
            type="submit"
