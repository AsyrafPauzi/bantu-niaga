...
function AgentAvatar({
  ...
  const Icon = ICON_BY_ID[agent.id] ?? Sparkles;
...
export function BoardroomExecutiveRoom({
  agents,
  activeCount,
}: {
  agents: BoardroomAgentState[];
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-4 lg:gap-6">
      <aside className="space-y-3 lg:col-span-1">
        <SectionCard
          ...
          subtitle="Only activated Marketplace agents join"
        ...
        <SectionCard title="Saved questions" subtitle="Quick re-runs" bodyClassName="space-y-2">
          {[
      ...
      <section className="space-y-3 lg:col-span-3">
        <p className="text-xs text-ink-muted dark:text-cream-400">
          Demo conversation below uses sample data. Live synthesis will use each
          active agent&apos;s module context ({liveAgents.map((a) => a.label).join(", ")}).
        </p>
        <Card>
            ...
            {CONVERSATION.map((entry, idx) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Sparkles
                        className="h-4 w-4 text-accent-700 dark:text-accent-200"
                        strokeWidth={2}
                      />
                    ...
                    <p className="mt-2 text-sm text-ink dark:text-cream-100">{entry.text}</p>
                  </div>
                );
              }
              ...
              return (
                <div key={idx} className="flex items-start gap-3">
                  {entry.agentId ? (
                    <AgentAvatar agentId={entry.agentId} agents={agents} />
                  ) : null}
                  <div className="min-w-0 flex-1">
        ...
        <Card>
          <CardBody>
            ...
            <p className="mt-2 text-xs text-ink-muted dark:text-cream-400">
              {activeCount} agents active in this room. Subscribe more agents in{" "}
