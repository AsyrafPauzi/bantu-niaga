...
export async function GET(request: Request) {
  ...
  const { data: addons, error } = await admin
    ...
    .eq("status", "active")
    .eq("marketplace_addons.slug", "sales-assistant");

  if (error) {
    logger.error("sales.notice.cron.load_failed", {
      error: error.message,
      requestId,
    });
  ...
  for (const row of addons ?? []) {
    ...
    const { data: settings } = await admin
      ...
      .eq("agent_slug", SALES_AGENT_SLUG)
      .maybeSingle();
    ...
    const displayName = settings?.display_name ?? "Sufi";

    try {
      const snapshot = await buildSalesSnapshot(
        {
          businessId,
          userId: owner.id,
          role: "owner",
      ...
      const notice = buildSalesDailyNotice(snapshot, displayName);
      ...
      if (upsertError) {
        logger.warn("sales.notice.cron.upsert_failed", {
          businessId,
      ...
      logger.warn("sales.notice.cron.business_failed", {
        businessId,
