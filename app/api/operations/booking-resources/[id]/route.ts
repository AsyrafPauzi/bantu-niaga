...
import { operationsBookingResourceUpdateSchema } from "@/lib/operations/schemas";
...
export async function PATCH(
  ...
  let parsed;
  try {
    parsed = operationsBookingResourceUpdateSchema.parse(body);
  } catch (e) {
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          message: status === 404 ? "Resource not found." : "Could not complete request.",
        },
      },
      { status },
    );
