...
const operationsAssistantSchema = z.object({
...
async function runOperationsAssistantChat(
...
export async function GET() {
  const { user, response } = await requireOperationsUser();
...
export async function POST(request: Request) {
  ...
  const rl = consume({
    bucket: "operations.assistant.chat",
  ...
  let parsed;
  try {
    parsed = operationsAssistantSchema.parse(body);
  } catch (error) {
  ...
  const [
    ...
    hasOperationsAssistantAddon(ctx.businessId),
    loadBusinessAgentSettings(ctx.businessId, OPERATIONS_AGENT_SLUG),
    getAgentCreditsSpentToday(ctx.businessId, OPERATIONS_AGENT_SLUG),
    supabase
      .from("businesses")
      .select("name")
    ...
    return NextResponse.json(
      {
        ...
        message: "Aiman is turned off in Settings → AI Agents.",
      },
      { status: 403 },
    );
  }
  ...
  const operationsPacketText = formatSnapshotPacket(operationsSnapshot);
  ...
  try {
    ...
    if (billable) {
      const spend = await spendCredits(ctx, {
        amount: chatCost,
        reason: "operations.assistant.chat",
      });
    ...
    logger.error("operations.assistant.failed", {
      businessId: ctx.businessId,
      error: detail,
    });
