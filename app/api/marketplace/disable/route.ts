...
/** POST /api/marketplace/disable — turn off feature; billing continues. */
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.rpc("marketplace_disable_addon_feature", {
  ...
  if (error) {
    ...
    if (msg.includes("owner role")) {
      return NextResponse.json(
        { error: "forbidden", message: "Only the business owner can disable add-ons." },
        { status: 403 },
      );
    }
