...
/** POST /api/marketplace/enable — turn feature back on; billing unchanged. */
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.rpc("marketplace_enable_addon_feature", {
  ...
  if (error) {
    ...
    if (msg.includes("owner role")) {
      return NextResponse.json(
        { error: "forbidden", message: "Only the business owner can enable add-ons." },
        { status: 403 },
      );
    }
