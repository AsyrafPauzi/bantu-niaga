...
export async function requireHrAttendanceAccess(): Promise<
...
> {
  try {
    ...
    const addonActive = await hasHrShiftAttendanceAddon(user.businessId);
    if (!addonActive) {
      return {
        user: null,
        response: NextResponse.json(
          {
            ...
            message:
              "Activate Shift Attendance in the Marketplace first.",
